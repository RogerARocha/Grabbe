...
| `/community` | `group` | Redirects to `/profile` |
| `/settings` | `settings` | Implemented (API Keys, Profile & Backup/Import) |
| `/profile` | `person` | Implemented (Unified Profile Card & Hall of Fame) |
...
* Guided setup wizard displayed when user profile or required API keys are missing.
* Interceptor in `App.tsx` redirects unconfigured sessions automatically.
* Form steps: Profile setup (Name, Rank Title, Avatar selection) → API Keys configuration (TMDB, IGDB) with inline validation button → Skip/Complete options.

### Profile & Hall of Fame

File: `src/pages/Profile.tsx`

* **Header:** Hero profile card with avatar, rank badge, and stat summary.
* **Hall of Fame:** User-selected top 4-5 media entries displayed with glowing borders and score badges.
* **Recap / Share Banner:** Generates exportable summary graphics (9:16 Story format or horizontal profile banner).
...
* Tabbed settings navigation:
  1. **General / Profile:** Edit display name and preferred default view.
